...
def read_metadata_old(meta_data_in, meta_data_out):
    sampleid = {}
    metainfo = {}
    samplerlen = {}
...
def update_metadata(meta_data_out, metainfo, sampleid, hashreads, hash16s, cellnum):
    with open(meta_data_out, 'w') as meta_out:  # 写模式，覆盖文件
        ...
        meta_out.write(f"{metainfo[sample_id]}\t{hashreads[sampleid[sample_id]]}\t"
                       f"{hash16s[sampleid[sample_id]]:.6f}\t{cellnum[sampleid[sample_id]]:.6f}\n")


def parse_args():
    parser = argparse.ArgumentParser(description="MetaGene analysis pipeline.")
    ...
    # parser.add_argument("--meta_data_in", help="Input metadata file.")
...
def main():
    ...
    # sampleid, metainfo, samplerlen = read_metadata_old(args.meta_data_in, args.meta_data_out)
    ...
    merge_fasta(args.outdir, sample, args.extracted_fasta)
