...
Taxonomy Abundance Merge Table and Stat Pipeline. 

Options:
    -prefix <s> : Required, the output prefix from step1
    -out    <s> : Required, output prefix
    -outdir <s> : Output Directory. Must be absolute path!!!
    -tax    <s> : Optional, path to tax.list file (default: $Tax)
    -help       : Show this help
USAGE
}

# 定义参数
my ($prefix, $outdir, $out, $help, $tax_arg);
GetOptions(
    "prefix:s" => \$prefix,
    "outdir:s" => \$outdir,
    "out:s"    => \$out,
...
# 如果提供了tax参数，则使用用户提供的值
$Tax = $tax_arg if defined $tax_arg;
